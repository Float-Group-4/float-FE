...
import './noborder.css';
import CreateProjectModal from '@pages/HomePage/CreateProject';
import { Project } from '../../../redux/project/projectSlice';

export default function CustomizedTables(projectList: Project[], rows: Data[]) {
  ...
  const [currentData, setCurrentData] = React.useState<Project>();
  ...
  const handleOpen = (_event: React.MouseEvent<unknown>, id: string) => {
    const value = projectList.find((p) => p.project.id == id);
    if (value != null) {
  ...
  return (
    <Box sx={{ width: '100%' }} className='content-center flex flex-col items-center mx-0 mr-5'>
      <Paper sx={{ width: '100%', mb: 2 }} elevation={0}>
        ...
        {isOpen && <CreateProjectModal isOpen={isOpen} setIsOpen={setIsOpen} data={currentData} />}
        <TableContainer>
          <Table
